...
int  My402ListAppend(My402List *list, void *obj){
    My402ListElem *node = NULL;//to ensure that the place we are pointing to in all null initialized(0) for the struct indicated that is obj=0,prev=0,next=0
    My402List *temporary = NULL ;//object value is 0 for anchor
    temporary = list;  
    ...
    if( My402ListEmpty(list)){
        ...
        node->obj = obj;//here we are passing the value of num_items
        temporary->anchor.next = node;//node contains address since its a pointer feed to A.next
        node->next = &(temporary->anchor);//get the address of anchor
        ...
        node->obj = obj;//load from argv
        node->next = &(temporary->anchor);//refer to diagram in web page. next will point to anchor so we need to know its address
        node->prev = temporary->anchor.prev;//prev will point to prev node but we want location wrt to the anchor
        temporary->anchor.prev->next = node;//wrt to anchor again indirection to reference to next
        temporary->anchor.prev = node;        
